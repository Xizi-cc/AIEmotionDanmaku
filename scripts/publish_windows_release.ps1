# Build AIEmotionDanmaku Windows x64 onedir and publish to release/AIEmotionDanmaku-windows-x64 (+ zip).
...
$ReleaseName = "AIEmotionDanmaku-windows-x64"
...
if (-not (Test-Path (Join-Path $DistDir "AIEmotionDanmaku.exe"))) {
    Write-Error "Missing $(Join-Path $DistDir 'AIEmotionDanmaku.exe') after build"
}
...
@(
    "AIEmotionDanmaku Windows x64 (PyInstaller onedir)"
    "Release folder: $ReleaseName"
    "Built (UTC): $builtAt"
    "Git: $gitSha"
    "Changelog: docs/CHANGELOG.md (2026-05-27)"
    ""
    "Run AIEmotionDanmaku.exe inside this folder. Requires WebView2 Runtime on Windows 10/11."
    "Fallback: AIEmotionDanmaku.exe --web-browser"
...
$exe = Join-Path $ReleaseDir "AIEmotionDanmaku.exe"
