...
Windows 发布包（PyInstaller onedir，`AIEmotionDanmaku.spec`）。

```powershell
.\scripts\build_exe.ps1
```

输出 `dist\AIEmotionDanmaku\AIEmotionDanmaku.exe`。需已存在 `data\danmu_pool_zh.json`。完整说明与问题记录见 [docs/PACKAGING_WINDOWS.md](../docs/PACKAGING_WINDOWS.md)。

## `publish_windows_release.ps1`

在 `build_exe.ps1` 之后，将 `dist\AIEmotionDanmaku\` 复制到本地发布目录（已 gitignore）并打 zip，供 GitHub Release 附件上传：
...
| 输出 | 说明 |
|------|------|
| `release\AIEmotionDanmaku-windows-x64\` | 完整 onedir（含 `AIEmotionDanmaku.exe`、`_internal\`） |
| `release\AIEmotionDanmaku-windows-x64.zip` | 同上，压缩包 |
