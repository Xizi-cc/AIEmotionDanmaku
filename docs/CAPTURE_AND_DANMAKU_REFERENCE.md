...
python-mss 和 Danmaku 都对 AIEmotionDanmaku 有参考价值，但当前不应直接替换现有 Qt 截图链路或 PyQt Overlay。
...
- AIEmotionDanmaku 的真实上屏必须走 PyQt 透明置顶窗口和 Win32 透明穿透能力。
- AIEmotionDanmaku 弹幕调度不只是渲染，还承担 AI 请求节奏、场景新鲜度、去重、批次清退。
