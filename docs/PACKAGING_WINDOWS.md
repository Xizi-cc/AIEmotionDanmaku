...
本文档记录 AIEmotionDanmaku 在 Windows 上打包为可分发 exe 的完整流程，以及实际打包过程中遇到的问题与对应修复。以仓库当前代码为准。

## 架构简述

```text
AIEmotionDanmaku.exe（主进程）
├─ PyQt6：弹幕 Overlay、系统托盘、截图
├─ 后台线程：uvicorn + FastAPI → http://127.0.0.1:18765
├─ 子进程：pywebview 桌面壳（WebView2 / edgechromium）
└─ 数据：%APPDATA%\AIEmotionDanmaku\（配置库，与源码运行共用）
...
| 路径 | 作用 |
|------|------|
| `AIEmotionDanmaku.spec` | PyInstaller 规格：入口、`datas`、`hiddenimports`、`excludes`、`console=False` |
...
| `%APPDATA%\AIEmotionDanmaku\startup.log` | **仅打包运行**时写入的启动诊断日志 |
...
若曾运行过 `dist\AIEmotionDanmaku\AIEmotionDanmaku.exe`，必须先结束，否则 PyInstaller 无法删除 `dist\AIEmotionDanmaku\`（`PermissionError: 拒绝访问`）。

```powershell
Get-Process AIEmotionDanmaku -ErrorAction SilentlyContinue | Stop-Process -Force
```

`build_exe.ps1` 也会自动尝试结束 `AIEmotionDanmaku` 进程。
...
| 输出 | 说明 |
|------|------|
| `dist\AIEmotionDanmaku\AIEmotionDanmaku.exe` | 主程序 |
| `dist\AIEmotionDanmaku\_internal\` | 依赖与打包资源（PyInstaller 6 onedir 布局） |
| `build\AIEmotionDanmaku\warn-AIEmotionDanmaku.txt` | 构建警告（缺失模块提示，供排查） |

**分发时必须提供整个 `dist\AIEmotionDanmaku\` 目录**（zip 或安装包），不能只拷贝单个 `AIEmotionDanmaku.exe`。

### 5. 本地验证

```powershell
.\dist\AIEmotionDanmaku\AIEmotionDanmaku.exe
```
...
- 弹幕 Overlay 正常
- `%APPDATA%\AIEmotionDanmaku\startup.log` 无新错误栈

可选：无 WebView2 或需排错时使用：

```powershell
.\dist\AIEmotionDanmaku\AIEmotionDanmaku.exe --web-browser
```
...
`AIEmotionDanmaku.spec` 中 `console=False`：无黑框控制台，适合桌面应用；但会引发下文「stderr 为 None」问题（已在代码中处理）。
...
- 失败诊断：`append_frozen_log()` → `%APPDATA%\AIEmotionDanmaku\startup.log`
...
**修复**：`AIEmotionDanmaku.spec` 中改为 `"web/static"`。
...
- `AIEmotionDanmaku.spec` 的 `EXCLUDES` 排除 `PyQt5`、`PySide2/6`、`IPython`、`pytest`、`jedi` 等
...
- 脚本打印 `Done: ...\AIEmotionDanmaku.exe`，但 PyInstaller 在 `COLLECT` 阶段已失败
- `PermissionError: [WinError 5] 拒绝访问` 删除 `dist\AIEmotionDanmaku\_internal\...`

**原因**：旧的 `AIEmotionDanmaku.exe` 仍在运行，DLL 被锁定；脚本未检查 `$LASTEXITCODE`。

**修复**：`scripts/build_exe.ps1` 增加：

- 构建前 `Stop-Process AIEmotionDanmaku`
- 构建前 `Remove-Item dist\AIEmotionDanmaku`
...
| 情况 | 识别方式 |
|------|----------|
| pywebview 正常 | 窗口标题多为 AIEmotionDanmaku，内嵌 WebView2，地址 `127.0.0.1:18765` |
| 回退系统浏览器 | 独立浏览器进程；`startup.log` 有 `fallback to system browser` |
| 强制浏览器模式 | 环境变量 `DANMU_WEB_LAUNCH=browser` 或 `AIEmotionDanmaku.exe --web-browser` |
...
**原因**：`AIEmotionDanmaku.spec` 使用 `console=False`（`runw.exe`），无控制台时 **`sys.stderr` 为 `None`**。`uvicorn.Config()` 默认配置 logging，`DefaultFormatter` 对 `stderr.isatty()` 调用导致崩溃，18765 从未监听。
...
**现象**：`AIEmotionDanmaku.exe` 为通用 Windows 程序图标；托盘为灰色圆角「D」占位图。

**原因**：仓库原先**未提交** `resources/icon.ico` 与 `resources/icon.png`（仅有 `resources/check.svg`）。`AIEmotionDanmaku.spec` 仅在文件存在时设置 exe 图标：
...
**现象**：`AIEmotionDanmaku.exe` 或 `python main.py` 后托盘图标出现，但看不到设置/控制台窗口；有时托盘「设置」也无反应。
...
1. `%APPDATA%\AIEmotionDanmaku\startup.log`（`pywebview start failed` / `fallback to system browser` / `web console not ready`）
2. `netstat -ano | findstr 18765`
3. `AIEmotionDanmaku.exe --web-browser` 或安装 [WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/)
...
```text
%APPDATA%\AIEmotionDanmaku\startup.log
```
...
若被占用，结束对应 PID 或关闭残留 `AIEmotionDanmaku.exe`。默认端口定义在 `app/web_console.py` 的 `DEFAULT_PORT = 18765`。

### 构建警告

```text
build\AIEmotionDanmaku\warn-AIEmotionDanmaku.txt
```

「missing module」多数为可选模块；若运行时报 `ModuleNotFoundError`，将模块名加入 `AIEmotionDanmaku.spec` 的 `hiddenimports`。

---

## 最终用户说明（可写入 Release）

1. 解压 **整个** `AIEmotionDanmaku` 文件夹后运行 `AIEmotionDanmaku.exe`。
2. 首次运行可在 Web「助手设置」配置 API；数据保存在 `%APPDATA%\AIEmotionDanmaku\`。
3. 需 **WebView2 Runtime**；若无，安装 Runtime 或使用 `AIEmotionDanmaku.exe --web-browser`。
4. 全局快捷键在部分环境需**管理员身份**运行。
5. 故障时提供 `%APPDATA%\AIEmotionDanmaku\startup.log`。
...
- [ ] `startup.log` 无新崩溃栈
- [ ] 分发 zip 含完整 `dist\AIEmotionDanmaku\` 目录
