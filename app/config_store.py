...
设计要点：
- **%APPDATA%/AIEmotionDanmaku/config.db** 持久化；**%APPDATA%/AIEmotionDanmaku/.key** 为 Fernet 对称密钥。
...
CONFIG_DIR = Path(os.environ.get("APPDATA", ".")) / "AIEmotionDanmaku"
...
class ConfigStore:
    ...
    def _init_fernet(self):
        """加载或生成 %APPDATA%/AIEmotionDanmaku/.key；校验失败则换新 key（旧密文永久不可读）。"""
        if not _HAS_CRYPTO:
