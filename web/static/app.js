/** AIEmotionDanmaku Web Console — 温馨 Qwen 原型 */
...
function buildDiagnosticReportText(diag) {
  ...
  return [
    'AIEmotionDanmaku Diagnostic Report',
    '',
    '[scheduler]',
...
function updateLogPanelState() {
  ...
  if (empty && visibleCount === 0) {
    if (REALTIME.logsOpen) {
      ...
      empty.textContent =
        '日志通道连接中… 若超过数秒仍无内容，请点左侧「温馨控制台」查看顶栏连接状态，或重启 AIEmotionDanmaku。';
    }
  }
}
...
async function init() {
  ...
  document.getElementById('previewImageFile')?.addEventListener('change', async (e) => {
    ...
    try {
      if (!API.token) {
        throw new Error('未获取会话令牌，请刷新页面或重启 AIEmotionDanmaku');
      }
      ...
      info.textContent = `${msg}（左侧为原图；请重启 AIEmotionDanmaku 后重试）`;
      if (compressedPh) {
...
init().catch((e) => {
  console.error(e);
  showToast(e.message || '无法连接小助手，请确认 AIEmotionDanmaku 已启动', true);
});
