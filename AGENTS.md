# AGENTS.md — AIEmotionDanmaku 桌面弹幕工具
...
| `app/config_store.py` | SQLite `%APPDATA%/AIEmotionDanmaku/config.db`，Fernet 加密 Key，WAL + 写锁 |
...
- **加密锁死**：丢失 `%APPDATA%/AIEmotionDanmaku/.key` → 已加密 Key 不可恢复
- **弹幕截断**：15 中文字 / 40 英文字 + `...`
